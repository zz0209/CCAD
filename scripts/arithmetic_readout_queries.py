...
def prepare_readouts(w, cfg):
    ...
    result, metadata = {}, []
    kinds = ['raw', 'activation'] + (['reconstruction'] if spec.get('include_reconstruction') else [])
    if spec.get('include_full_activation'):kinds.append('full_activation')
    for record in query['banks'][0]['records']:
        ...
        with np.load(path) as z:
            ...
            for op in ['unit', 'tens']:
                ...
                if 'full_activation' in kinds:shared[op]['full_activation']=torch.tensor(z[f'{op}_full_activation'],device=w.device)
        ...
        metadata.append(dict(source_seed=s, target_seed=t, path=path.as_posix(), sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
                             ...
                             reconstruction_input='full target code difference decoded with the target SAE, then the frozen raw predictor' if 'reconstruction' in kinds else None,
