...
def main():
    ...
    from native_participation_paper import plot as plot_participation
    plot_participation(data.get('native_participation'),save)
    from ravel_semantics_paper import plot as plot_ravel
    ...
    if (paper/'data/reform_r22.json').exists():
        from reform_paper import plot as plot_reform
        plot_reform(paper)
        for ext in ['pdf','svg','png']:
    ...
    if (paper/'data/reform_r23.json').exists():
        ...
        for ext in ['pdf','svg','png']:
            ...
            outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/reform_r24.json').exists():
        from behavior_paper import plot as plot_behavior
        plot_behavior(paper)
        for ext in ['pdf','svg','png']:
            p=paper/'figures'/('behavior_native_groups.'+ext)
            outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/reform_r25.json').exists():
        from grammar_paper import plot as plot_grammar
        plot_grammar(paper)
        for name in ['grammar_source_effects','grammar_transfer_errors','grammar_controlled_probes']:
            for ext in ['pdf','svg','png']:
                p=paper/'figures'/(name+'.'+ext)
                outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/reform_r26.json').exists():
        from component_transfer_paper import plot as plot_components
        plot_components(paper)
        for name in ['component_effects','component_transfer_errors']:
            for ext in ['pdf','svg','png']:
                p=paper/'figures'/(name+'.'+ext)
                outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/final_value_r27.json').exists():
        from functional_reuse_paper import plot as plot_functional_reuse
        plot_functional_reuse(paper)
        for ext in ['pdf','svg','png']:
            p=paper/'figures'/('functional_reuse.'+ext)
            outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/final_value_r28.json').exists():
        from functional_reuse_paper import plot_path
        plot_path(paper)
        for ext in ['pdf','svg','png']:
            p=paper/'figures'/('functional_path.'+ext)
            outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/final_value_r29.json').exists():
        from independent_consensus_paper import plot as plot_independent
        plot_independent(paper)
        for name in ['independent_consensus','functional_regions','functional_unions']:
            for ext in ['pdf','svg','png']:
                p=paper/'figures'/(name+'.'+ext)
                outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/final_value_r30.json').exists():
        from external_functional_paper import plot as plot_external
        plot_external(paper)
        for ext in ['pdf','svg','png']:
            p=paper/'figures'/('external_functional_queries.'+ext)
            outputs.append(dict(path=p.relative_to(paper).as_posix(),bytes=p.stat().st_size,sha256=hashlib.sha256(p.read_bytes()).hexdigest()))
    if (paper/'data/final_value_r31.json').exists():
        from response_query_paper import plot as plot_response_queries
        plot_response_queries(paper)
        for ext in ['pdf','svg','png']:
