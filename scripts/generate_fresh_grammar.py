...
def main():
    ...
    for task in tasks:
        ...
        cls=ns['AgreementGenerator' if task.startswith('regular') else 'AnaphorGenerator'];gen=cls()
        rows=[];counts=Counter()
        for attempt in range(a.count*200):
            ...
            assert good!=bad and good.startswith(data['one_prefix_prefix']) and bad.startswith(data['one_prefix_prefix'])
    ...
    manifest=dict(written_at_utc=datetime.now(timezone.utc).isoformat(),files=files,seed=a.seed,
        ...
        split_rule='First16 accepted pairs per task are calibration; next128 evaluation. No model outcomes used for generation or exclusion.',
