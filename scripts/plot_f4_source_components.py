...
def main(compact=False):
    ...
    assert len(rows)==(195 if compact else 75) and all(math.isfinite(v) and v>0 for v in values)
    lo=math.floor(math.log10(min(values)));hi=max(0,math.ceil(math.log10(max(values))))
    im=Image.new('RGB',(2400,1740 if compact else 1440),'white');d=ImageDraw.Draw(im);points=[];blue='#0072B2';black='#202020'
    ...
    text(1200,30,'One shared16 interface transfers both source parts' if compact else 'Source-defined parts retain real effects across recipient seeds',44,'ma',True)
    text(1200,91,'One common 16-member target support for A and B | Frozen source parts | Exposed development' if compact else 'Frozen source rank1 basis | Two discovery-ranked 16-member groups | Exposed development',28,'ma')
    if compact:
    ...
    for col,metric in enumerate(metrics):
        left=305+1180*col;width=835;top=257;bottom=1320 if compact else 1092
    ...
    for i,line in enumerate(footer):text(55,(1460 if compact else 1203)+i*40,line,24)
...
if __name__=='__main__':main('--compact' in sys.argv)
