...
def main():
    ...
    code=[]
    for rel in ['scripts/replay_f4_and_targets.py','scripts/f4_interpretation_endpoint.py','scripts/run_r011s1_raw_hook_asset.py','src/ccad/artifacts.py','src/ccad/activation_contract.py']:
    ...
    write(run/'manifest.json',dict(schema_version='fcc.and.targets.v1',run_id=cfg['run_id'],run_parent='F4',purpose='Frozen cross-seed prediction of a source output contrast, with atom/raw controls',milestone='C2-C3-interpretability',evidence_level=cfg.get('evidence_level','source_selected_authored_cross_seed_feasibility'),
        ...
        mean_constants_source_split='independent mean; exact cancellation in paired difference',threshold_source_split='frozen source config; target behavior not used for selection',statistics_unit=cfg.get('statistics_unit','one source query, four target seeds, four topics across two families; dependent'),device=cfg['device'],seeds=[1,2,3,4,5],resource_lease='gpu-0 resource_manager.run',resource_lease_reason='224 bounded forwards and small cached-hook SAE encoding; no fitting or whole-disk work'))
    ...
    try:
        ...
        n=len(saved)
        if not 0<n<=16 or n%2 or len(auth['cases'])!=n:raise ValueError('Invalid full source scope')
        ...
        hooks=np.stack([arr[f'hook_{i}'] for i in range(n)]);coefficients={};target_codes={};raw_coefs={}
        for t in cfg['target_seeds']:
            ...
            z=np.zeros((n,3072));np.add.at(z,(np.arange(n)[:,None],enc.top_indices.cpu().numpy()),enc.top_acts.cpu().numpy());target_codes[t]=z
        ...
        for i,c in enumerate(auth['cases']):
            ...
            basecontrast=contrast(base,clause,items);sourcecontrast=contrast(source,clause,items)-basecontrast
            ...
            for t in cfg['target_seeds']:
                ...
                for method,pred in predicted.items():
                    p,_=forward(c['token_ids'],-pred*b*scale);effect=contrast(p,clause,items)-basecontrast
                    kl=float(np.sum(source*(np.log(np.maximum(source,1e-300))-np.log(np.maximum(p,1e-300)))))
                    cm,im=masses(p,clause,items)
                    row=dict(c,run_id=cfg['run_id'],metric_version='v1',target_seed=t,method=method,predicted_coordinate=pred,source_coordinate=scalar,dose_scale=scale,source_contrast=sourcecontrast,candidate_contrast=effect,
                        contrast_error=effect-sourcecontrast,clause_mass=cm,item_mass=im,source_to_candidate_kl=kl,source_to_baseline_kl=den,normalized_kl_error=kl/den if den>1e-12 else None)
                    rows.append(row)
