...
def main():
    ...
    for rel in ['scripts/run_composition_material.py','scripts/composition_panel.py','scripts/run_functional_material.py','scripts/run_r011s1_raw_hook_asset.py','src/ccad/factor_correspondence.py','src/ccad/nip_baselines.py','src/ccad/proposal.py','src/ccad/artifacts.py','src/ccad/activation_contract.py']:
    ...
    try:
        ...
        positions=np.array([[r['region_ends'][i] for i in [0,2,4]] for r in tokenrows]);label_tokens=[tokenizer.encode(s,add_special_tokens=False) for s in cfg['labels']];assert all(len(s)==1 for s in label_tokens);labels=np.array([s[0] for s in label_tokens]);checks['token_alignment']=True
        write(run/'panel.json',dict(rows=rows,pairs=pairs,token_rows=tokenrows,labels=cfg['labels'],label_ids=labels.tolist(),position_names=['time','number','distractor'],scope=cfg['scope']))
        captures={l:np.empty((n,3,model.config.hidden_size),np.float32) for l in cfg['layers']}
            ...
            def make_hook(l):
                ...
                def hook(mod,args,out):
                    ...
                    if delta is not None and layer==l:
                        h=h.clone()
                        for slot in range(3):h[ix,torch.as_tensor(positions[ids,slot],device='cuda')]+=torch.as_tensor(delta[:,slot],device='cuda')
        ...
        if cfg.get('sae_checkpoints'):
            ...
            for spec in cfg['sae_checkpoints']:
                ...
                del sae;np.savez_compressed(run/f"seed{spec['seed']}_codes.npz",time_z=z[:,0],number_z=z[:,1]);variants={name:np.zeros_like(h,dtype=np.float64) for name in ['source_native16','source_native32','source_full_sae','source_pca1']};arrays={}
        ...
        checks['all_rows']=len(metrics)==n*(1+len(cfg['layers'])*len(specs)+len(cfg.get('sae_checkpoints',[]))*12);checks['finite']=all(np.isfinite(v) for r in metrics for v in r.values() if isinstance(v,float));checks['unique']=len(metrics)==len({(r['kind'],r.get('layer'),r.get('seed'),r.get('method'),r.get('factor'),r['row_id']) for r in metrics})
