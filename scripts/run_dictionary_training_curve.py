...
def main():
 ...
 try:
  ...
  for split in ['train','validation']:
   info=token_manifest['outputs'][split];p=Path(info['path']);assert sha256(p)==info['sha256'];inputs.append(entry(p,'Pinned-model tokenized natural corpus',split,'ODC-By-1.0'));data[split]=np.memmap(p,dtype=info.get('dtype',token_manifest.get('token_dtype','<u2')),mode='r').reshape(-1,128)
  for name in ['config.json','tokenizer.json','model.safetensors']:inputs.append(entry(Path(cfg['model_local_dir'])/name,cfg['model_id']+' '+cfg['model_revision'],'base model',cfg.get('model_license','MIT')))
