...
def main():
    ...
    write(run/'manifest.json',dict(schema_version='fcc.source.components.v1',run_id=cfg['run_id'],run_parent='F4',
        ...
        threshold_source_split='source-only discovery group ranking; exposed cases unchanged',statistics_unit='query/document/shared-seed dependencies',
    ...
    try:
        ...
        fresh=fitconfigs['new_fit'];base=load(fresh['source_asset_config']);ev=load(fresh['evaluation_asset_config'])
        ...
        raw={r['split']:mmap(r) for r in raw_manifest['splits'] if r['split'] in ('mean','discovery')}
        eval_raw=mmap(next(r for r in eval_raw_manifest['splits'] if r['split']=='calibration'))
        rawmean=np.asarray(raw['mean'],dtype=float).mean(axis=0)
        ...
        for s in sorted({s for s,a in queries}):
            init_codes('source',s,source_manifest,('mean','discovery'));init_codes('evalsource',s,eval_manifest,('calibration',))
        ...
        for s,a in queries:
            ...
            w,diag=additive_ridge(np.asarray(raw['discovery'][rr],dtype=float)-rawmean,y,ww,cfg['ridge_fraction'])
        ...
        for case_id,e in enumerate(cases):
            ...
            for t in cfg['target_seeds']:
                if t!=s:coords[f'long_{t}']=embed((z_eval[t][[rowindex[int(r)] for r in rr]]-z_eval[t][[rowindex[int(r)] for r in dd]])@coefficients[s,a,t])
        ...
        for e in compiled:
            ...
            for name in ['source','raw']+[f'long_{t}' for t in cfg['target_seeds'] if t!=e['source_seed']]:
                for component in ('full','A','B'):
        ...
        checks.update(noop_exact=max(noop)==0,cached_hook_replay=max(replay)<=1e-4,expected_forwards=forwards==cfg['expected_forwards'],all_cases=len(compiled)==cfg['expected_cases'],all_rows=len(records)==18*cfg['expected_cases'])
