...
def main():
    ...
    cfg=json.loads(args.config.read_text());compact=cfg.get('compact_shared_interface',False);run=ROOT/'runs'/cfg['run_id'];run.mkdir(exist_ok=False)
    ...
    try:
        ...
        if compact:
            ...
            frozen_parts={(r['source_seed'],r['source_atom']):r for r in load(parent/'component_fits.json',frozen['fits_sha256']) if r['kind']=='source_partition'}
        ...
        if compact:
            for t in cfg['target_seeds']:
                init_codes('short',t,source_manifest,('mean','discovery'));init_codes('evalshort',t,eval_manifest,('calibration',))
