...
def main():
    ...
    cfg = json.loads((ROOT / task["asset_config"]).read_text(encoding="utf-8"))
    cfg.update(task)
    ...
    write(run / "code_hashes.json", {"files":code, "aggregate_sha256":code_hash})
    ...
    try:
        ...
        selections=[]
        for s,a in queries:
            targets=[(s-1+j)%5+1 for j in (1,2)]
        ...
        write(run/"selection.json",{"rule":"source-only hashes and activation energies; no F4 FOUND filtering","queries":selections})
        ...
        checks={"noop":max(noop)<=1e-6,"raw_replay_relative":max(replay)<=1e-4,"eight_source_queries":len(selections)==8,"rows":len(rows)==sum(len(x["sequences"])*len(x["targets"])*len(cfg["ranks"])*4 for x in selections),"audit_closed":True}
        ...
        for condition in ("positive","negative"):
            for rank in cfg["ranks"]:
                for method in ("target","raw","wrong_query","wrong_query_matched_energy"):
