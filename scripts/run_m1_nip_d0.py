...
from ccad.nip_synthetic import FAMILIES, generate_nip_observed, observed_kernels

ROOT = Path(__file__).resolve().parents[1]
SOURCES = ("src/ccad/mscc.py", "src/ccad/nip_synthetic.py", "scripts/run_m1_nip_d0.py")
...
def finalize_failure(run_dir: Path, exc: BaseException) -> None:
    """Persist a terminal failure for any exception after run-directory creation."""
...
def main() -> int:
    ...
    if not config["execution_enabled"] or config["phase"] != "D0" or config["truth_opened"] or config["held_out_eval_opened"] or config["real_sae_audit_opened"]:
        raise ValueError("D0 config violates the closed-information contract")
    if tuple(config["families"]) != FAMILIES or config["pairs_per_family"] != 5:
        raise ValueError("D0 family grid must match the frozen protocol")
    ...
    config_row = snapshot_file(config_path, run_dir, "input_snapshot", "configs/m1_nip_d0_v1.json")
    ...
    write_json(run_dir / "manifest.json", {
        "artifact_schema_version": "ccad.run.v1", "run_id": run_dir.name,
        "tracker_parent": "M1_NIP_protocol_v1", "purpose": "D0 truth-closed engineering",
        "milestone": "M1", "evidence_level": config["evidence_level"],
        "started_utc": started, "started_local": started_local,
        "trigger": "automation heartbeat ccad", "operator": "Codex", "project_root": str(ROOT),
        "config_hash": sha(run_dir / "config.resolved.json"), "code_snapshot_hash": code_hash,
        "protocol_sha256": protocol_row["sha256"], "execution_config_sha256": config_row["sha256"],
        "git_available": False, "model_id": "not_applicable", "model_revision": "not_applicable",
        "dataset_id": "synthetic_m1_nip_d0", "dataset_revision": "generated_from_snapshotted_code",
        "tokenizer_id": "not_applicable", "sae_framework": "not_applicable",
        "device": "cpu", "resource_lease": "not_required_lightweight_cpu",
        ...
        "split": "synthetic_discovery_D0", "statistical_unit": "seed_pair",
        "audit_opened": False, "candidate_family_frozen": True,
        "mean_constants_source_split": "synthetic_declared_mean", "threshold_source_split": "frozen_protocol",
        "artifact_schema": ["manifest", "config", "environment", "inputs", "code_hashes", "status", "logs", "raw_metrics", "summary", "source_snapshot", "input_snapshot"],
        "protocol_deviations": [],
    })
    records = []
    for family in FAMILIES:
        for pair_index in range(5):
            ...
            observed = generate_nip_observed(family, structural_seed=seeds["structural"], sample_seed=seeds["sample"], n=config["observations_per_pair"])
            ...
            records.append({"run_id": run_dir.name, "family_id": family, "pair_index": pair_index, "statistical_unit": "seed_pair", "metric_version": "m1_nip_d0.v1", "seeds": seeds, "source_shape": observed.source_contributions.shape, "target_shape": observed.target_contributions.shape, "proposal_status": proposal.status, "proposal_hash": proposal.proposal_hash, "prediction_hash": frozen.prediction_hash, "full_dictionary_comparisons": proposal.full_dictionary_comparisons, "planned_support_count": proposal.planned_support_count, "evaluated_count": result.evaluated_count, "truth_opened": False, "held_out_eval_opened": False})
    raw = run_dir / "metrics.raw.jsonl"
    raw.write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in records), encoding="utf-8")
    runner_hash = next(row["sha256"] for row in source_rows if Path(str(row["snapshot"])).as_posix().endswith("scripts/run_m1_nip_d0.py"))
    summary = {"status": "PASS", "semantic_outcome": "D0_ENGINEERING_ONLY_NO_LABEL_SCORING", "record_count": len(records), "family_count": len(FAMILIES), "pairs_per_family": 5, "raw_sha256": sha(raw), "generation_script_sha256": runner_hash, "maximum_planned_support_count": max(row["planned_support_count"] for row in records), "proposal_refusal_count": sum(row["proposal_status"] != "OK" for row in records), "truth_opened": False, "held_out_eval_opened": False, "real_sae_audit_opened": False}
    write_json(run_dir / "metrics.summary.json", summary)
    (run_dir / "stdout.log").write_text("D0 truth-closed engineering completed\n", encoding="utf-8")
