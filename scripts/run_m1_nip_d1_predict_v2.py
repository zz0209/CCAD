"""Truth-blind M1-NIP v2 prediction and closure writer."""
from __future__ import annotations

import argparse, hashlib, json, os, platform, shutil, sys, traceback
...
SOURCES = ("src/ccad/mscc.py", "src/ccad/nip_synthetic.py", "src/ccad/nip_synthetic_v2.py", "scripts/run_m1_nip_d1_predict_v2.py")
...
def snapshot(source: Path, run_dir: Path, prefix: str, relative: str) -> dict:
...
def fail(run_dir: Path, exc: BaseException) -> None:
    if run_dir.is_dir() and not (run_dir / "prediction_closure.json").exists():
        write_json(run_dir / "status.json", {"status": "FAIL", "ended_utc": now(), "failure_type": type(exc).__name__, "failure_message": str(exc)})
        (run_dir / "stderr.log").write_text("".join(traceback.format_exception(exc)), encoding="utf-8")
...
def main() -> int:
    ...
    if not config["execution_enabled"] or config["phase"] not in {"I1", "D1"} or config["truth_opened_in_prediction"] or config["held_out_eval_opened"] or config["real_sae_audit_opened"]:
        raise ValueError("prediction information boundary is open")
    if tuple(config["families"]) != FAMILIES or config["atom_caps"] != [4, 8, 12, 16, 20] or config["target_atom_count"] != 20:
        raise ValueError("grid differs from v2 protocol")
    protocol = ROOT / config["protocol_path"]
    if sha(protocol) != config["protocol_sha256"]:
    ...
    started = now()
    write_json(run_dir / "manifest.json", {"artifact_schema_version": "ccad.prediction_run.v1", "run_id": run_dir.name, "phase": config["phase"], "evidence_level": config["evidence_level"], "started_utc": started, "protocol_sha256": config["protocol_sha256"], "code_snapshot_hash": code_hash, "truth_opened": False, "formal_d1_seed_consumed": config["formal_d1_seed_consumed"], "statistical_unit": "seed_pair", "caps_paired_within_seed_pair": True})
    write_json(run_dir / "status.json", {"status": "RUNNING", "started_utc": started})

    records = []
    for family in FAMILIES:
        for pair in range(config["pairs_per_family"]):
            seeds = {stream: seed_for(config["protocol_sha256"], code_hash, config["phase"], family, pair, stream) for stream in ("structural", "sample", "proposal", "solver")}
            ...
            for cap in config["atom_caps"]:
                ...
                records.append({
                    ...
                    "protocol_hash": frozen.protocol_hash, "discovery_fingerprint": fingerprint, "source_atom_id": 0,
                    "proposal": {"status": proposal.status, "source_atom_id": 0, "atom_cap": cap, "g_max": config["g_max"], "candidate_budget": config["candidate_budget"], "boundary_tie_tolerance": config["boundary_tie_tolerance"], "ranking": sorted(range(20), key=lambda i: (proposal.singleton_d_ctr[i], i)), "singleton_d_ctr": proposal.singleton_d_ctr, "proposed_target_ids": proposal.proposed_target_ids, "boundary_margin": proposal.boundary_margin, "planned_support_count": proposal.planned_support_count, "full_dictionary_comparisons": proposal.full_dictionary_comparisons, "refusal_reason": proposal.refusal_reason, "proposal_hash": proposal.proposal_hash},
                    "prediction": {"search_status": result.status, "identification": result.identification, "multiplicity": result.multiplicity, "minimum_support_size": result.minimum_support_size, "supports": [asdict(item) for item in result.supports], "best_candidate": None if result.best_candidate is None else asdict(result.best_candidate), "nearest_competitor": None if result.nearest_competitor is None else asdict(result.nearest_competitor), "planned_candidate_count": result.planned_candidate_count, "evaluated_count": result.evaluated_count, "complete_universe": result.complete_universe, "unresolved_reason": result.unresolved_reason, "elapsed_seconds_descriptive_only": result.elapsed_seconds, "prediction_hash": frozen.prediction_hash},
                    "truth_opened": False,
                })
    raw = run_dir / "predictions.raw.jsonl"
    raw.write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in records), encoding="utf-8")
    write_json(run_dir / "predictions.summary.json", {"status": "PASS", "record_count": len(records), "truth_opened": False, "raw_sha256": sha(raw), "runtime_used_for_selection": False})
    (run_dir / "stdout.log").write_text("sealed truth-blind predictions\n", encoding="utf-8")
    (run_dir / "stderr.log").write_text("", encoding="utf-8")
    write_json(run_dir / "status.json", {"status": "PASS", "started_utc": started, "ended_utc": now()})
    bound = ["predictions.raw.jsonl", "predictions.summary.json", "config.resolved.json", "code_hashes.json", "inputs.json", "environment.json", "manifest.json", "status.json"]
    closure = {"schema_version": "ccad.prediction_closure.v1", "state": "SEALED", "run_id": run_dir.name, "row_count": len(records), "protocol_sha256": config["protocol_sha256"], "code_snapshot_hash": code_hash, "files": {name: sha(run_dir / name) for name in bound}}
