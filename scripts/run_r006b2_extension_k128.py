"""R006-B2 k=128 extension with fail-closed reuse of the initial bracket."""
...
from ccad.sae_quality import select_k_extension  # noqa: E402
...
REUSE_FIELDS = [
    "model_id", "model_revision", "tokenizer_revision", "model_local_dir", "model_license", "sparsify_commit",
    ...
    "dataset_commit", "dataset_license", "compute_environment_canonical_sha256", "compute_environment_source_sha256",
    "context_length", "max_train_sequences", "max_validation_sequences", "train_tokens",
    "batch_size_sequences", "eval_batch_size_sequences", "layer_index", "hookpoint", "hook_module_path",
    "hook_oracle_hidden_state_index", "num_latents", "learning_rate", "lr_warmup_steps",
    "dead_feature_threshold", "auxk_alpha", "exclude_token_ids", "init_seeds", "data_order_seed",
    "precision", "attn_implementation", "device", "cublas_workspace_config", "decoder_cosine_block_size",
    "selection_fve_margin", "selection_ce_recovered_margin",
]
...
def main() -> int:
    ...
    input_specs = [
        ...
        (reuse_dir / "inputs.json", cfg["reuse_run"], "internal immutable run artifact", "reuse_inputs"),
        ...
        (reuse_dir / "checkpoints/k_64/state.pt", cfg["reuse_run"], "internal immutable run artifact", "reuse_k64_exact_checkpoint"),
        (reuse_dir / "saes/k_64/sae.safetensors", cfg["reuse_run"], "internal immutable run artifact", "reuse_k64_safe_sae"),
    ]
    inputs = [file_entry(*item) for item in input_specs]
    ...
    write_json(run_dir / "manifest.json", {
        ...
        "resource_lease_reason": "Pythia-160M k128 extension training and full intervention evaluation",
    ...
    try:
        ...
        if sha256(spec_path) != cfg["compute_environment_source_sha256"] or canonical_json_hash(spec_path) != cfg["compute_environment_canonical_sha256"]:
            raise ValueError("compute environment hash mismatch")
        ...
        critical_paths = (
            "scripts/run_r006b2_topk_bracket.py", "scripts/run_r006b_topk_capacity.py",
            "src/ccad/checkpointing.py", "src/ccad/activation_contract.py", "src/ccad/decoder_diagnostics.py",
        )
        current_hashes = {path: sha256(ROOT / path) for path in critical_paths}
        reuse_checks = {
            "source_status_pass": reuse_summary["status"] == "PASS" and reuse_summary["selection_decision"] == "EXPAND_TO_64_128",
            ...
            "source_suite_checks": all(reuse_raw["suite_checks"].values()),
            "critical_code_unchanged": all(historical_hashes[path] == digest for path, digest in current_hashes.items()),
            "fixed_config_compatible": all(reuse_cfg[field] == cfg[field] for field in REUSE_FIELDS),
            "source_candidate_set_exact": sorted(map(int, reuse_raw["candidates"])) == [8, 16, 32, 64],
            "source_input_trace_exact": all(
                reuse_raw["candidates"][str(k)]["train_input_hashes"] == expected_input_hashes
                for k in reuse_cfg["train_k"]
            ),
            "source_k64_checkpoint_hash": sha256(reuse_dir / "checkpoints/k_64/state.pt") == reuse_raw["candidates"]["64"]["checkpoint_state_sha256"],
        }
        if not all(reuse_checks.values()):
            raise RuntimeError(f"initial bracket reuse failed: {reuse_checks}")
        ...
        initial_hash = state_hash(sae.state_dict())
        source_initial_hash = reuse_raw["candidates"]["64"]["initial_state_hash"]
        ...
        candidate_checks = {
            ...
            "initial_state_matches_source": initial_hash == source_initial_hash,
        ...
        new_candidate = {
            "source": "trained_in_extension", "checks": candidate_checks, "sae_name": sae_name,
            "initial_state_hash": initial_hash, "final_state_hash": state_hash(sae.state_dict()),
        ...
        selector_inputs = {
            int(key): {"fve": item["validation"]["fve"], "ce_recovered": item["validation"]["ce_recovered"], "c_dec": item["c_dec"]}
            for key, item in reuse_raw["candidates"].items()
        }
        selector_inputs[k] = {"fve": evaluation["fve"], "ce_recovered": evaluation["ce_recovered"], "c_dec": c_dec}
        selection = select_k_extension(selector_inputs, cfg["selection_fve_margin"], cfg["selection_ce_recovered_margin"])
        suite_checks = {
            ...
            "initial_bracket_reuse_checks": all(reuse_checks.values()),
            "candidate_checks": all(candidate_checks.values()),
            "candidate_set_exact": sorted(selector_inputs) == cfg["candidate_k"],
            "selector_has_valid_decision": selection["decision"] in {"TWO_SEED_PILOT", "UNBOUNDED_HIGH", "UNBOUNDED_GEOMETRY", "NO_JOINT_ELIGIBLE"},
        }
        record = {
            "suite_checks": suite_checks, "reuse_checks": reuse_checks, "new_candidate": new_candidate,
            "source_candidates": reuse_raw["candidates"], "selector_inputs": selector_inputs, "selection": selection,
            "expected_input_hashes": expected_input_hashes,
            "reuse_critical_code_hashes": {"historical": {path: historical_hashes[path] for path in critical_paths}, "current": current_hashes},
    ...
    write_json(run_dir / "metrics.summary.json", {
        ...
        "generator_script_path": "scripts/run_r006b2_extension_k128.py", "generator_script_sha256": code_entries[0]["sha256"],
    })
