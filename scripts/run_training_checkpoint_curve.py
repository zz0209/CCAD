...
def main():
 ...
 try:
  ...
  env=dict(python=sys.executable,python_version=platform.python_version(),torch=torch.__version__,transformers=transformers.__version__,gpu=torch.cuda.get_device_name(),matmul_precision=torch.get_float32_matmul_precision(),hidden_size=model.config.hidden_size,layer_index=cfg['layer_index'])
  def seed_for(name):
