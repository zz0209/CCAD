...
def main():
    ...
    result=dict(written_utc=datetime.now(timezone.utc).isoformat(),training=training,quality_by_step=quality,function_run=dict(run=function['run'],summary=function['summary'],raw_sha256=function['raw_sha256']),source_function=fun,transfer_changes=changes,fixed_teacher_label_distributions_exact=True,fixed_legacy_and_raw_kl_max_error=max(abs(r['kl_difference']) for r in invariant),continuation=json.loads((args.training/'continuation_phase.json').read_text()),scope='Training continues on a new4M natural-token phase with restarted LR. Same-stream prefix is not retrained. Natural quality uses original256 validation sequences; source function reselects groups on old384 temporal prompts. Transfer alone holds the4M source teachers and all inputs fixed while replacing target codes. All evaluation remains exposed development, five dependent cyclic directions.')
