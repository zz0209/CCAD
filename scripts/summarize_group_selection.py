...
def describe(run):
 ...
 selectors=sorted(choices[0]['selected']);records=[];metrics={};candidate_records=[];all_method_records=[]
 for c in choices:
  for name in c['held_summary']:
   ar=[r for r in qrows[c['query']] if r['method']==name]
   a=dict(query=c['query'],task=c['task'],objective=c['objective'],source_seed=c['source_seed'],target_seed=c['target_seed'],method=name,rows=len(ar),iia=c['held_summary'][name]['iia'],iia_flip=float(np.mean([r['iia'] and r['base_correct'] for r in ar])),base_correct_fraction=float(np.mean([r['base_correct'] for r in ar])))
   for key in ['log_odds_ratio','donor_ce','source_kl']:a[key]=float(np.mean([r[key] for r in ar])) if ar and all(key in r for r in ar) else None
   all_method_records.append(a)
  for s in selectors:
   ...
   best=c['scores'][s][order[0]];ties=[n for n in order if abs(c['scores'][s][n]-best)<=1e-10]
   rec.update(tied_candidates=len(ties),tie_uniform_top1_iia=float(np.mean([c['held_summary'][n]['iia'] for n in ties])))
   records.append(rec)
  for name,cs in c['candidate_stats'].items():
   actual_rows=[r for r in qrows[c['query']] if r['method']==name]
   candidate_records.append(dict(query=c['query'],task=c['task'],objective=c['objective'],method=name,iia=c['held_summary'][name]['iia'],iia_flip=float(np.mean([r['iia'] and r['base_correct'] for r in actual_rows])),log_odds_ratio=float(np.mean([r['log_odds_ratio'] for r in actual_rows])),donor_ce=float(np.mean([r['donor_ce'] for r in actual_rows])) if all('donor_ce' in r for r in actual_rows) else None,source_kl=float(np.mean([r['source_kl'] for r in actual_rows])) if all('source_kl' in r for r in actual_rows) else None,**{k:cs[k] for k in ['members','l1','relative_realization_l2','natural_mse']}))
 for s in selectors:
  rr=[r for r in records if r['selector']==s];metrics[s]={key:float(np.mean([r[key] for r in rr if r[key] is not None])) for key in ['top1_iia','tie_uniform_top1_iia','top3_uniform_iia','ranking_spearman','regret'] if any(r[key] is not None for r in rr)}
 ...
 fixed={m:float(np.mean([c['held_summary'][m]['iia'] for c in choices])) for m in sorted(set.intersection(*(set(c['held_summary']) for c in choices)))}
 budgets=[dict(query=c['query'],task=c['task'],objective=c['objective'],source_seed=c['source_seed'],target_seed=c['target_seed'],**r,held_iia=c['held_summary'][r['selected']]['iia']) for c in choices for r in c.get('budget_choices',[])]
 return dict(run=run.name,config=cfg,queries=len(choices),metrics=metrics,records=records,candidate_records=candidate_records,all_method_records=all_method_records,fixed_method_iia=fixed,budget_records=budgets,source_iia=float(np.mean([c['source_held_iia'] for c in choices])),raw_iia=float(np.mean([c['held_summary']['raw']['iia'] for c in choices])),full_target_iia=float(np.mean([c['held_summary']['full_target']['iia'] for c in choices])),source_files=[dict(path=str(run/f),sha256=digest(run/f)) for f in ['config.resolved.json','selection_choices.json','metrics.raw.jsonl','metrics.summary.json']])
def main():
