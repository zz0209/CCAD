"""Frozen early-late confirmation with all methods and whole-pair contrasts."""
import json,hashlib
...
def main():
 r=ROOT;out=r/'artifacts/training_gain_confirmation_20260906';cfg=json.loads((r/'configs/f4_training_gain_confirmation_v1.json').read_text());quality={(x['seed'],x['step']):x for x in [json.loads(t) for t in (r/'runs/R012_same_stream_curve_v1_20260906/metrics.raw.jsonl').read_text().splitlines()]};records=[];arrays={};reference=None;identities=[]
 for cell in cfg['cells']:
  run=r/'runs'/cell['run'];summary=json.loads((run/'metrics.summary.json').read_text());assert summary['status']=='PASS';rr=[json.loads(t) for t in (run/'metrics.raw.jsonl').read_text().splitlines()];rows=[x for x in rr if x['method'] in cfg['original_methods']];lookup={(x['method'],x['operator'],x['case_id']):x for x in rows};methods=cfg['original_methods'];ops=sorted({x['operator'] for x in rows});ids=sorted({x['case_id'] for x in rows});pairs=sorted({x['pair'] for x in rows});assert len(rows)==len(methods)*len(ops)*len(ids) and len(ids)==2*len(pairs) and 0<len(pairs)<=12
