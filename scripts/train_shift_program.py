"""Warm-start a target dictionary to preserve requests in a source program.
...
def main():
    ...
    w=MultisiteWork(c,args.config,['scripts/train_shift_program.py','scripts/run_shift_explanation.py','scripts/train_shift_dictionaries.py','scripts/run_causalgym_multisite.py','scripts/run_r011s1_raw_hook_asset.py','src/ccad/artifacts.py'])
    hooks=[];error=None
    try:
        ...
        initial={};targets={};indices={};coefficients={};q={}
        for s in sites:
        ...
        write(w.run/'evaluation_membership.json',dict(split='dev',rows=dev))
        program_rows=[]
        ...
        def evaluate(name):
            nonlocal mode,mask
            mode='source' if name=='source' else ('clean' if name=='none' else 'student')
            for query in (['full'] if name=='none' else c['queries']):
                set_query(query);values=np.empty(len(dev),dtype='float32')
                ...
                for off in range(0,len(order),c['eval_batch_size']):
                    ...
                    _,po=forward(ids);values[ix]=(po@pw.T+pb).squeeze(-1).cpu().numpy()
                for row,value in zip(dev,values):
                    w.record(kind='classification',task='profession',row_id=row['row_id'],component=row['document_sha256'],method=name,operation=query,seed=c['target_seed'],target_seed=c['target_seed'],split='dev',label=row['label'],gender=row['gender'],prediction=int(value>0),logit=float(value))
        ...
        calibration=[];calgen=torch.Generator(device=w.device).manual_seed(917)
        with torch.no_grad():
        ...
        for variant in c['variants']:
            for s in sites:
                ...
                coefficients[s].requires_grad_(variant in ('whole','parts','parts_relation','random_parts'))
            for s in c['adapt_sites']:targets[s].requires_grad_(variant!='parts_relation')
            groups_opt=[dict(params=[p for p in params if p.requires_grad],lr=c['dictionary_lr'])]
            if variant in ('whole','parts','parts_relation','random_parts'):groups_opt.append(dict(params=list(coefficients.values()),lr=c['relation_lr']))
            ...
            for step in range(c['steps']):
                ...
                if variant in ('whole','parts','parts_relation','random_parts'):
                    if program_rows:
                    ...
                    if variant in ('parts','parts_relation') and c.get('request_training')=='semantic_singletons':
                        set_query(list(groups)[step%len(groups)])
                    else:
                        for s in sites:q[s]=torch.ones_like(q[s]) if variant=='whole' else (torch.rand(q[s].shape,generator=generator,device=w.device)<.5).float()
                    mode='source'
                    with torch.no_grad():teacher_h,teacher_pool=forward(ids)
                    mode='student';student_h,student_pool=forward(ids)
                    energy,pe=scales[:2]
                ...
                with torch.no_grad():
                    for s in c['adapt_sites']:
                        if variant!='parts_relation':targets[s].decoder.weight.div_(targets[s].decoder.weight.norm(dim=0).clamp_min(1e-10))
