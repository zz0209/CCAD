...
def main():
    ...
    try:
        ...
        def evaluate(name):
            nonlocal mode,mask
            mode=name if name in ('source','geometry','geometry_gain','raw','raw_reconstruction') else ('clean' if name=='none' else 'student')
        ...
        for variant in c['variants']:
            for s in sites:
                ...
                coefficients[s].requires_grad_(True)
            ...
            groups_opt.append(dict(params=list(coefficients.values()),lr=c['relation_lr']))
