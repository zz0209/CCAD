...
def main() -> int:
    ...
    expected_inputs = {"protocol", "diagnostic_config", "execution_config"} | ({"selection_freeze"} if closure.get("selection_freeze_sha256") else set())
    check("input_keyset", set(inputs) == expected_inputs)
    ...
    check("selection_freeze_binding", config["phase"] != "D2" or config["selection_freeze_sha256"] == closure["selection_freeze_sha256"] == inputs["selection_freeze"]["sha256"])
