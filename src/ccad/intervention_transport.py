...
def allocate_columns(z, columns, decoder, allowance, active_only=True):
...
def transport_delta(h, target, source, q, write_matrix, allowance, active_only=True):
    ...
    allocated = allocate_columns(z, columns, target.decoder.weight, allowance, active_only)
    dz = allocated @ q
    return dz @ target.decoder.weight.T, dz, allocated


def refine_columns(h, target, source, columns, allowance, steps=64, write_matrix=None):
    ...
    lipschitz=torch.linalg.eigvalsh(gram)[...,-1].clamp_min(1e-8)
    divisor=torch.arange(1,p+1,device=h.device,dtype=h.dtype)
    for _ in range(steps):
        proposal=a-(gram@a-rhs)/lipschitz[:,None,None]
        ...
        a=proposal.clamp_min(0)-(negative-tau).clamp_min(0)
